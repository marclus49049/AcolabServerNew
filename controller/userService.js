...
const getUser = async (req, res, next) => {
	try {
		...
		const user = await User.findById(req.user.id, function (error, data) {
			res.json(data);
		});
	} catch (e) {
			...
			if (err) {
				...
				if (expiredate > d) {
					...
					if (data.webinarlist.includes(req.body.webinarid)) {
						// Send URL here name it meeting_url
						res.status(202).json({
...
const leaderboard = async (req, res, next) => {
	...
	try {
		...
		if (req.body.action in scoreSystem) {
			...
			User.findOneAndUpdate({ _id: req.user.id }, user, function (err, res) {});
			res.json(user);
		} else {
